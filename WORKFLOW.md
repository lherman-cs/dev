...
Spec and Review use one local, session-scoped webpage for human interaction. Entering either phase opens the page and shows a local URL; `/dev-spec-view` or Alt+S and `/dev-review-view` or Alt+R reopen it. The page presents one attention item at a time in a focused packet: inspect changed subjects in a pending revision before application, decide a consequential item with a visible recommendation, wait for a contextual answer, then submit the exact-target review. A compact progress count and remaining-work line stay visible; evidence, code, system context and version-anchored discussion open in a closed-by-default context drawer. Resolving an item advances to the next unresolved item and focuses its heading. Choosing an alternative requests a revised assessment rather than accepting the recommendation; waiving a disclosed risk requires confirmation. Applying a revision never approves it. The command palette and keyboard shortcuts support quick navigation. Agents publish structured semantic decision context and optional predefined visual data, never frontend markup. Existing explanations and evidence require no model call; asking the agent does. A change request blocks approval until a new revision is published and applied. The agent edits `plans/<project>/spec.md` and publishes its semantic content for Spec; Review publishes a candidate-bound engineering assessment. No terminal approval fallback is provided. Outside these phases, structured human questions still use `ask_user_question`.
