...
| Ship | Fix a local comparison base and candidate scope. Map the resulting behavior and consequential choices, then walk through one concrete topic at a time with the human, following their questions rather than a fixed approval checklist. Repair clear defects and discuss consequential changes before implementation. Once the candidate settles, obtain one independent review, close meaningful findings with affected validation, and seek explicit human confirmation that feedback is exhausted. Only then refine owned history with a recovery reference and leave a clean committed local candidate. |
...
Agent Hub is a view and owner-action adapter, not a scheduler. The foreground Shipper sequences the human walkthrough, independent review of the settled candidate, repairs, validation, explicit convergence confirmation, and history cleanup. The Hub preserves independent drafts, anchored history navigation, contextual help, native messages, tool rendering, and completed child transcripts. A queued instruction is not delivered until accepted. The worker owner seals completed sessions and rejects stale structured results after interventions. Related questions create a fresh bounded investigation rather than reviving a completed result.
