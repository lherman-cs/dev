...
`dev a resume` opens Pi's session picker; `dev a resume <session-id-or-path>` opens that session directly. The public phases are exactly `spec`, `build`, and `ship`. With no prompt, `dev a <phase>` opens the configured interactive Pi session. A prompt invokes the matching current-session `/dev-<phase>` alias. Each alias explicitly loads its `dev-<phase>` skill. The active agent owns its independent worktree through alignment, implementation, review, and local history refinement. Build needs an approved outcome, which may be established in conversation without a spec file. Ship is a topic-by-topic human walkthrough of resulting behavior and consequential decisions. After the candidate settles, the Shipper obtains one independent read-only review, closes meaningful findings, asks explicitly whether feedback remains, then refines local history. See [WORKFLOW.md](WORKFLOW.md) for the review and recovery sequence. Integration and remote publishing belong to the human.
