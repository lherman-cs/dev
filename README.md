...
`dev a resume` opens Pi's session picker; `dev a resume <session-id-or-path>` opens that session directly. The public phases are exactly `spec`, `build`, and `ship`. With no prompt, `dev a <phase>` opens the configured interactive Pi session. A prompt invokes the matching current-session `/dev-<phase>` alias. Each alias explicitly loads its `dev-<phase>` skill. The active agent owns its independent worktree through alignment, implementation, review, and local history refinement. Build needs an approved outcome, which may be established in conversation without a spec file. Ship opens with a concise, decision-ready assessment of the resulting behavior, rationale, evidence limits, risks, and recommendation; discussion follows the human's questions rather than a fixed tour. After the candidate settles, the Shipper obtains one broad independent read-only review with deep attention to the highest risks, closes meaningful findings itself, and uses at most one human-approved repair audit when a concrete coverage need remains. It then presents the consolidated assessment, asks explicitly whether review is complete, and refines local history. See [WORKFLOW.md](WORKFLOW.md) for the review and recovery sequence. Integration and remote publishing belong to the human.
