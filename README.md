...
Brief is a read-only, optional communication phase, independent of Spec/Build/Review/Ship. `dev brief` captures the merge-base of the locally discoverable default branch and `HEAD` against the effective current tree (staged, unstaged, untracked; ignored files excluded). `--base <branch>` selects another local base; `--range A..B` compares the exact committed endpoint trees and `--range A...B` compares their merge-base to B, excluding working edits. No remote fetch, checkout, staging of the user's index, or commit occurs. If the default branch is missing or ambiguous, supply `--base`. `--spec <path>` adds explicit intent; it is not treated as evidence of implementation. The saved revision, bounded captured input, and feedback live outside the repository under XDG data storage. `--list` lists revisions and `--open <id|latest>` reopens without regeneration. The localhost viewer renders an agent-authored Markdown brief with constrained `brief-lead` and `brief-section` JSON directives for text, lists, icon/status lists, tables, before/after comparisons and transitions, flows, timelines, annotated architecture paths, Mermaid diagrams, pseudocode/excerpts, callouts, and explicit responsive columns. The illustrative, source-controlled [reference sample](tests/fixtures/brief-reference.md) is a full Markdown document rendered by the same parser and viewer; its SCReAM-like story is not a claim about this repository. Sections and navigation follow the evidence rather than a fixed template. Generation can read eligible source from the frozen comparison endpoint, including unchanged files, through bounded read-only tools; ignored and known credential paths are excluded. The saved Markdown is parsed before persistence; unsupported or ungrounded directives get up to two same-snapshot correction attempts, then fail without replacing a revision. Existing saved revisions retain their original content and notes; listing uses compact sidecars for new revisions. The viewer restores local unsaved drafts, detects same-comment conflicts across tabs, marks stale snapshots, and exports only confirmed Markdown feedback with revision IDs and snapshot-relative code/spec anchors. A general note explicitly has no code target. The local server lasts until `Ctrl+C`; commenting and exporting use the viewer, which requires a browser. Generation prints capture, Pi, and save stages to stderr while the final URL stays on stdout.
