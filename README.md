...
## Agent Hub\n\n`Alt+A` opens one session-wide Agent Hub for every child `AgentSession` created by this workflow package, including Explorers spawned directly from the normal main conversation. The main console keeps a compact live-agent widget; the Hub provides a live roster + inspector, `Enter` thread drill-in, direct steering, `Alt+↑/↓` thread switching, follow-up, history scrolling, and per-worker stop. Context-sensitive key hints are always visible in the Hub footer, so the interaction is discoverable without separate docs. `/dev-workers` opens the same surface. Controllers still own workflow state and verification; the Hub only observes and interacts with child sessions.\n\nSkills use **Pi's native lazy-skill mechanism**. Only skill names/descriptions are discoverable up front; the full `SKILL.md` body is injected only by an explicit `/skill:dev-*` invocation. Bare `dev a <phase>` therefore does not preload phase instructions into the session. Fresh workers discover only the one assigned skill and invoke it natively for that task.
