...
dev a ship "Review and refine the local media-signaling candidate"
dev a resume                       # interactive Pi session picker
dev a stats                        # interactive Pi session dashboard
```

`dev a resume` opens Pi's session picker; `dev a resume <session-id-or-path>` opens that session directly. The public phases are exactly `spec`, `build`, and `ship`. With no prompt, `dev a <phase>` opens the configured interactive Pi session. A prompt invokes the matching current-session `/dev-<phase>` alias. Each alias explicitly loads its `dev-<phase>` skill. The active agent owns its independent worktree through alignment, implementation, review, and local history refinement. Build needs an approved outcome, which may be established in conversation without a spec file. Ship opens with a concise engineering assessment of the practical improvement, design rationale, concrete risks, evidence limits, and recommendation; discussion follows the human's questions rather than a fixed tour. After the candidate settles, the Shipper obtains one broad independent read-only review with deep attention to the highest risks, closes meaningful findings itself, and uses at most one human-approved repair audit when a concrete coverage need remains. It then presents the consolidated assessment, obtains explicit authorization to finish local review and clean up local commits (not integrate or deploy), and refines local history. Failed independent scrutiny is disclosed as a separate risk decision when material; passing checks cannot replace it. See [WORKFLOW.md](WORKFLOW.md) for the review and recovery sequence. Integration and remote publishing belong to the human.

`explore` starts a separate bounded Explorer child session in its own HEAD snapshot for investigation. Uncommitted owner edits are not in that snapshot; inspect them as owner or supply explicit read-only evidence. Parent agents delegate substantial read-only investigation, while quick known-target reads may stay direct. Explorer may investigate but does not run checks, edit, make project decisions, or delegate. `verify({ commands, cwd?, prerequisites?, timeoutMs? })` dispatches tests, builds, lints, benchmarks, and acceptance gates against the actual owner worktree, including dirty files. It returns immediately with a receipt; asynchronous evidence includes candidate fingerprint, exits, duration, excerpts, and retained logs. Keep the candidate unchanged until the check completes or is cancelled. Verifier does not repair, install, or mutate Git. During an active `dev-ship` effort only, `review({ purpose, task, candidate, evidence, focus })` can use its single broad-review slot or single repair-audit slot. Candidate preflight creates an immutable snapshot containing tracked and untracked non-ignored worktree content and rejects stale identity before model execution. Each review has a five-minute deadline; its bounded receipt persists in the ship goal across session recovery and history-only commit changes. Every child-agent launch returns a job receipt immediately and delivers its bounded result separately as soon as it settles. This includes worker-owned Explorers and Reviewer launches. Callers never await child agents: they continue independent work, treating a result as evidence rather than an ownership transfer or prerequisite for recovery. If Explorer fails or is unavailable, a parent may disclose an output-bounded direct read-only investigative fallback, but gains no otherwise prohibited capability. The active Shipper verifies the local candidate identity and evidence, and obtains explicit human review convergence confirmation.
