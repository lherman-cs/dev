...
| Consequential human decisions | Spec obtains explicit semantic approval; build resolves ordinary implementation choices; review repairs in-scope bugs and obtains human approval for all consequential candidate changes; ship has no semantic decision authority. |
| Review is the senior engineering phase | `dev-review` directly reviews and repairs the existing candidate diff; the human owns integration and commits; no Reviewer sub-agent owns judgment. |
