...
export const ciCheckSchema = Type.Object({ id: text(500), name: text(500), head: oid, state: text(100), conclusion: Type.Union([text(100), Type.Null()]), appId: Type.Optional(Type.Integer()) }, { additionalProperties: false });
...
export const waitOutcomeSchema = Type.Object({ status: Type.Union([Type.Literal("passed"), Type.Literal("failed"), Type.Literal("stale_timeout"), Type.Literal("cancelled"), Type.Literal("interrupted")]), candidate: candidateIdentitySchema, requiredCheckIds: Type.Optional(Type.Array(text(500))), checks: Type.Array(ciCheckSchema), observedSignals: Type.Optional(Type.Array(text(500))), reason: Type.Optional(text(500)) }, { additionalProperties: false });
