...
export async function waitForCi(candidate: CandidateIdentity, requiredIds: readonly string[], deps: CiPollDependencies, config: CiPollConfig, signal?: AbortSignal, expectedSignals: readonly string[] = []): Promise<WaitOutcome> {
  ...
  const finish = async (status: WaitOutcome["status"], reason?: string): Promise<WaitOutcome> => { const result: WaitOutcome = { status, candidate, requiredCheckIds: [...requiredIds], checks: latest, ...(expectedSignals.length ? { observedSignals } : {}), ...(reason ? { reason } : {}) }; await deps.persist(result); return result; };
  try {
    for (;;) {
