...
**Stop condition:** Continue by default. Stop only when completing shipment requires a consequential scope, ownership, product-semantics, target, destructive-history, or authority decision that cannot be safely resolved from the request, applicable approved decisions, repository/GitHub evidence, and reasonable engineering judgment, or when an external dependency required for completion remains unavailable after feasible retries. Ask one precise question stating what decision or access is required, why it is consequential, and why proceeding autonomously would be unsafe.
