...
The outer `text` fence above illustrates the format, not part of the response. Emit the document beginning at `# Branch consequence brief`. There can be zero or more sections; only include materially useful sections. Each section has 1–12 blocks. All blocks require `type`, `label`, `anchors` and their listed fields. Allowed blocks:
