...
export function FinishPacket({ current, phase, remaining, blockers, waiting, gate, approved, canApprove, active, onReview, onNext }: { current?: string; phase: "spec" | "review"; remaining: number; blockers: number; waiting: number; gate?: string; approved: boolean; canApprove: boolean; active: boolean; onReview: () => void; onNext: () => void }) {
  return <div className="mt-8 space-y-5"><p className="text-base leading-7">{!current ? "The agent is preparing a focused question or a working draft. You can send context here while you wait." : approved ? "This exact target is approved. No next phase was started." : remaining ? `${remaining} consequential decision${remaining === 1 ? "" : "s"} remain before approval.` : waiting ? `${waiting} question${waiting === 1 ? "" : "s"} waiting for the agent before approval.` : blockers ? `Approval unavailable: ${gate}.` : "Review the working summary and your decisions before approving this exact target."}</p>
