...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (previousTarget.current && previousTarget.current !== target) { setReview(false); setWaiver(false); setNotice(data.state.pending ? "" : "The review target changed. Inspect the current state before acting.") }
      previousTarget.current = target
      const latestReply = [...data.state.discussions].reverse().find(m => m.author === "agent")?.id || ""
      if (lastReply.current && latestReply && latestReply !== lastReply.current) setNotice("Agent replied. Your answer and the updated proposal are ready to inspect.")
  ...
  const version = current?.version || workspace?.state.prompt?.decision.version || "unassessed"
  ...
  const decision = (current?.decisions || (phase === "spec" && workspace?.state.prompt ? [workspace.state.prompt.decision] : [])).find(d => item.kind === "decision" && d.id === item.subject)
  ...
  const canDecide = !!workspace?.active && !pending && !error && (phase === "spec" && !current && !!workspace?.state.prompt || ["Current candidate", "Current spec", "Resolve consequential decisions"].includes(workspace.gate))
  ...
  const alternative = () => { const option = decision?.options?.find(choice => choice.id === selectedOption); if (decision && option && !busy && (canRequest || phase === "spec" && !current && canDecide)) void act({ action: !current ? "submit" : "request_changes", subject: decision.id, text: `Choose alternative for ${decision.subject}: ${option.label}. ${option.summary}`, version }, "Alternative sent. The agent will reflect it in the proposed spec.") }
  ...
  return <WorkspaceShell items={entries} selected={item.id} onSelect={choose} project={workspace?.project || "Workspace"} phase={phase} summary={progress} dark={dark} toggleTheme={() => setDark(!dark)} commands={() => setPalette(true)} overview={() => setQueue(true)} finish={() => choose("finish")}>
    {(notice || error || !pending && workspace?.state.notice) && <p role="status" aria-live="polite" className="mb-7 border-l-2 border-primary px-3 text-sm">{error || notice || workspace?.state.notice}</p>}
    <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground"><span>{item.kind === "decision" ? `${phase === "spec" ? "Spec" : "Review"} decision` : phase === "spec" ? "SPECIFICATION" : "IMPLEMENTATION REVIEW"} {decision?.kind === "risk" && " · Disclosed risk"}</span><button className="quiet-link" onClick={() => setQueue(true)}><List size={15}/> {phase === "spec" ? "Spec overview" : "Review overview"}</button></div>
    <p className="eyebrow mt-6">{item.kind === "revision" ? "PROPOSED REVISION" : item.kind === "decision" ? "QUESTION FOR YOU" : item.kind === "waiting" ? "WAITING FOR AGENT" : current ? "FINAL CHECK" : "GETTING STARTED"}</p>
    ...
    {pending && item.kind !== "revision" && <div className="mt-5 rounded-lg border border-ring bg-accent/40 p-4 text-sm"><strong>Proposal updated</strong><p className="mt-1">Your draft stays here. Inspect the changed target before accepting a decision.</p><button type="button" className="quiet-link mt-2" onClick={() => choose(`revision:${pending.version}`)}>Inspect what changed →</button></div>}
    {decision && !current && workspace?.state.prompt && <p className="mt-6 text-base leading-7">{workspace.state.prompt.motivation}</p>}
    {decision && <DecisionPacket decision={decision} phase={phase} disabled={!canDecide} disabledReason={pending ? "Inspect and apply the changed proposal before accepting this answer." : !workspace?.active ? "Waiting for the agent to resume." : undefined} busy={busy} selectedOption={selectedOption} setSelectedOption={id => setSelectedOptions(options => ({ ...options, [decision.id]: id }))} accept={accept} alternative={alternative} waive={() => setWaiver(true)} depth={openDepth} composer={<form onSubmit={send} className="space-y-3"><label htmlFor="own-answer" className="text-sm font-medium">Or give your own answer or ask a question</label><Textarea id="own-answer" value={draft} onChange={e => updateDraft(e.target.value)} rows={3} maxLength={20000} placeholder="Explain what you need, or challenge the recommendation"/><div className="flex flex-wrap items-center gap-3"><Button type="submit" variant="outline" disabled={!workspace?.active || !!error || busy || !draft.trim() || draftVersion !== version}>Send to agent</Button><span className="text-xs text-muted-foreground">Sending does not accept the recommendation.</span></div>{draft && draftVersion !== version && <p className="text-sm">Your draft belongs to a different revision. <button type="button" className="quiet-link" onClick={() => { setDraftVersion(version); if (workspace) save(workspace.workspace, `version:${item.subject}`, version) }}>I rechecked this question</button></p>}{comments.filter(m => m.author === "human").slice(-1).map(m => <p key={m.id} className="text-sm text-muted-foreground">Your {m.status === "queued" ? "answer is with the agent" : m.status === "failed" ? "answer was not delivered" : "answer is recorded"}: {m.text}</p>)}{comments.filter(m => m.author === "agent").slice(-1).map(m => <div key={m.id} className="rounded-lg border bg-card p-4 text-sm" aria-label="Agent reply"><strong>Agent replied</strong><Prose text={m.text}/></div>)}</form>}/>}
    ...
    {item.kind === "finish" && current && <section className="mt-8 space-y-6" aria-label="Working summary"><div><h2 className="text-lg font-semibold">What you are approving</h2><p className="mt-2 text-sm text-muted-foreground">{current.recommendation}</p></div>{current.sections.filter(section => section.kind === "scope" || section.kind === "requirement" || section.kind === "outcome" || section.kind === "risk").map(section => <div key={section.id} className="border-t pt-4"><h3 className="font-medium">{section.title}</h3><div className="mt-2 text-sm"><Prose text={section.body}/></div></div>)}{current.decisions.length > 0 && <div className="border-t pt-4"><h3 className="font-medium">Decisions on this version</h3><ul className="mt-2 space-y-2 text-sm">{current.decisions.map(d => <li key={d.id}>{d.subject}: <strong>{d.status === "open" ? "Needs your answer" : d.status === "waived" ? "Risk waived" : "Accepted"}</strong></li>)}</ul></div>}{phase === "spec" && workspace?.state.discussions.some(m => m.version === "unassessed" || workspace.state.promptVersions?.includes(m.version)) && <div className="border-t pt-4"><h3 className="font-medium">Earlier clarification</h3><div className="mt-2 space-y-2 text-sm">{workspace.state.discussions.filter(m => m.version === "unassessed" || workspace.state.promptVersions?.includes(m.version)).map(m => <p key={m.id} className="break-words text-muted-foreground"><strong>{m.author === "human" ? "You" : "Agent"}:</strong> {m.text} {m.status && `· ${m.status}`}</p>)}</div></div>}</section>}
    {item.kind === "finish" && <><FinishPacket current={current?.candidate?.head || current?.version} phase={phase} remaining={progress.remaining} blockers={progress.blockers} waiting={progress.waiting} gate={workspace?.gate} approved={approved} canApprove={canApprove} active={!!workspace?.active && !busy && !error} onReview={() => { setOutcome(canApprove ? "approve" : "comment"); setReview(true) }} onNext={() => choose(entries.find(entry => entry.kind === "decision" && !entry.done)?.id || entries[0].id)}/>{!current && !decision && <form onSubmit={send} className="mt-6 space-y-3"><label htmlFor="early-question" className="text-sm font-medium">Ask the agent about the desired outcome</label><Textarea id="early-question" value={draft} onChange={e => updateDraft(e.target.value)} rows={3} maxLength={20000}/><Button type="submit" disabled={!workspace?.active || !!error || busy || !draft.trim()}>Send question</Button></form>}</>}
