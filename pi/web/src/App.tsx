...
function itemsFor(workspace: Workspace | null): Item[] {
  ...
  if (doc) {
    for (const d of doc.decisions) items.push({ id: `decision:${d.id}`, kind: "decision", title: d.subject, subject: d.id, done: d.status !== "open" })
...
export default function App() {
  ...
  const [dark, setDark] = useState(() => { try { return localStorage.getItem("workspace-theme") === "dark" || (!localStorage.getItem("workspace-theme") && matchMedia("(prefers-color-scheme: dark)").matches) } catch { return false } })
  ...
  const defaultItem = entries.find(i => !i.done && i.kind !== "question") || entries.find(i => !i.done) || entries.at(-1)!
  ...
  const blockers = Number(Boolean(pending)) + Number(Boolean(current && !pending && workspace?.gate !== "Current spec" && workspace?.gate !== "Current candidate" && workspace?.gate !== "Resolve consequential decisions" && workspace?.gate !== "Spec file changed outside the assessment" && workspace?.gate !== "No assessment has been published" && workspace?.gate !== "No spec revision published"))
  ...
  useEffect(() => { const onKey = (e: KeyboardEvent) => {
    ...
    if (k === "j" || k === "n") { e.preventDefault(); next() }
    if (k === "k" || k === "p") { e.preventDefault(); previous() }
    if (k === "w") setDepth("why")
    if (k === "e") setDepth("evidence")
    if (k === "c") setDepth("code")
    if (k === "a" && decision?.status === "open" && canDecide) setConfirm({ action: "decide", id: decision.id, status: "accepted", version })
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <header className="border-b"><div className="mx-auto flex max-w-3xl items-center justify-between gap-3 px-5 py-3 text-xs sm:px-8"><div className="min-w-0 truncate font-medium">{workspace?.project || "Workspace"} <span className="mx-1 text-muted-foreground">/</span> {phase === "spec" ? "Spec" : "Review"} <span className="ml-2 font-normal text-muted-foreground">{short(version)}</span></div><div className="flex shrink-0 items-center gap-1"><button type="button" className="icon-button" aria-label="Open commands" title="Commands · Ctrl/⌘ K" onClick={() => setPalette(true)}><Command size={16}/></button><button type="button" className="icon-button" aria-label={dark ? "Use light theme" : "Use dark theme"} onClick={() => setDark(!dark)}>{dark ? <Sun size={16}/> : <Moon size={16}/>}</button></div></div></header>
    <main className="mx-auto max-w-3xl px-5 pb-28 pt-8 sm:px-8 sm:pt-12">
      ...
      <p className="mt-4 border-b pb-4 text-sm font-medium" aria-label="Remaining work">{open} decision{open === 1 ? "" : "s"} left <span className="mx-2 text-muted-foreground">·</span> {blockers} blocker{blockers === 1 ? "" : "s"} unresolved <span className="mx-2 text-muted-foreground">·</span> {waiting} question{waiting === 1 ? "" : "s"} waiting</p>
      ...
      <div className="mt-9"><p className="eyebrow">{item.kind === "revision" ? "REVISION TO INSPECT" : item.kind === "decision" ? decision?.kind === "risk" ? "DISCLOSED RISK" : "YOUR DECISION" : item.kind === "question" ? "WAITING FOR THE AGENT" : "FINISH LINE"}</p>
        ...
        <p className="mt-3 text-sm text-muted-foreground">{item.kind === "revision" ? `Proposed ${short(pending?.version)} · Current ${short(version)}` : item.kind === "finish" ? `${phase === "spec" ? "Spec" : "Candidate"} · exact revision ${short(version)}` : subjectName}</p>
      </div>
      {item.kind === "decision" && decision && <div className="mt-9 space-y-7">
        ...
        <div><p className="eyebrow">MINIMUM CONTEXT</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{phase === "spec" ? "This choice shapes the durable specification. Accepting asks the agent to incorporate it before approval." : "This choice is tied to the current assessed candidate. Approval remains a separate, exact-target action."}</p></div>
