...
test('six symmetric commands, worker hub and a narrow Explorer do no work during registration', async () => {
  const commands=new Map(), handlers=new Map(), messages=[], tools=[];
  extension({registerCommand:(n,c)=>commands.set(n,c),registerTool:t=>tools.push(t),registerEntryRenderer:()=>{},appendEntry:()=>{},on:(n,h)=>handlers.set(n,h),sendUserMessage:(...a)=>messages.push(a),exec:async()=>({code:0,stdout:'',stderr:''})});
