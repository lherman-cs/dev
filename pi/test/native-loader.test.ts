...
test('native Pi loads pinned plugins, four aliases and skills without errors', async t => {
  ...
  for (const tool of ['explore', 'review', 'todo', 'ask_user_question', 'lsp_diagnostics', 'lsp_fix', 'chrome_devtools_load', 'web_search', 'fetch_content', 'mcp']) assert.ok(tools.includes(tool), tools.join(','));
