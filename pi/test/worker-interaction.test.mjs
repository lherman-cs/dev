...
async function fixture(t, handler, { history = false, createHook } = {}) {
  ...
  t.after(() => { if (previous === undefined) delete process.env.PI_CODING_AGENT_DIR; else process.env.PI_CODING_AGENT_DIR = previous; fs.rmSync(root, { recursive: true, force: true }); });
  ...
  t.after(async () => { await hub.stopAll(); hub.dispose(); });
  runtime.streamSimple = (model, context, requestOptions) => {
    ...
    const finish = message => stream.push({ type: "done", reason: message.stopReason, message });
...
const verdict = (model, value = "pass") => response(model, "", [{ type: "toolCall", id: `result-${Math.random()}`, name: "submit_result", arguments: { verdict: value } }], "toolUse");
...
test("actual SDK: Main Explorer receives ordinary steering, records delivery and retains native history", { timeout: 15000 }, async t => {
  ...
  assert.equal(await pending, "follow-up addressed");
  ...
  const records = f.hub.history.records(); const saved = records.find(r => r.id === id);
...
test("actual SDK: a pre-intervention structured result is rejected at settlement", { timeout: 15000 }, async t => {
  ...
  held.finish(response(held.model, "old wrap-up")); await rejected;
  assert.equal(f.hub.list()[0].state, "failed");
});

test("actual SDK: a new structured result after delivery is accepted, but silent review PASS is not", { timeout: 15000 }, async t => {
  const ready = deferred();
  const f = await fixture(t, call => {
    if (call.n === 1) return verdict(call.model);
    if (call.n === 2) { ready.resolve(call); return; }
...
test("lifetime cancellation while extensions bind cannot start a prompt afterward", { timeout: 15000 }, async t => {
  ...
  const pending = f.run({ cwd: f.cwd, name: "build", task: "Wait for binding" });
...
test("stopping a Builder cancels its active nested Explorer and preserves lineage", { timeout: 15000 }, async t => {
  ...
  const f = await fixture(t, call => {
    if (call.model.id.includes("sol")) return response(call.model, "", [{ type: "toolCall", id: "investigation", name: "explore", arguments: { task: "Inspect the tests" } }], "toolUse");
    call.options.signal.addEventListener("abort", () => call.stream.push({ type: "error", reason: "aborted", error: { ...response(call.model, "", [], "aborted"), errorMessage: "cancelled" } }), { once: true });
    nested.resolve();
  });
  const pending = f.run({ cwd: f.cwd, name: "build", task: "PRIVATE_PARENT_TEXT", skill: "dev-implement" });
  const rejected = assert.rejects(pending, /stopped|cancel|abort/i);
  await nested.promise;
  const parent = f.hub.list().find(r => r.role === "build"), child = f.hub.list().find(r => r.role === "explorer");
  assert.equal(child.metadata.parentId, parent.id);
  assert.ok(!JSON.stringify(f.calls[1].context.messages).includes("PRIVATE_PARENT_TEXT"));
  await f.hub.abort(parent.id); await rejected;
...
test("a native worker waits for explicit human input; inspection alone cannot answer", { timeout: 15000 }, async t => {
  ...
  await asked.promise; assert.equal(f.calls.length, 1);
  let showed = false;
...
test("VCC loads in the child and its native registered tools are not hidden by the whitelist", { timeout: 15000 }, async t => {
  const f = await fixture(t, call => response(call.model));
  await f.run({ cwd: f.cwd, name: "explorer", task: "One answer" });
  const extensions = f.options[0].resourceLoader.getExtensions().extensions;
  const vcc = extensions.find(e => e.resolvedPath.includes("pi-vcc"));
  assert.ok(vcc, "VCC must be explicitly loaded, not inferred from Main");
  const tools = [...vcc.tools.keys()];
  assert.ok(tools.length > 0, "pinned VCC should expose its recall tool");
