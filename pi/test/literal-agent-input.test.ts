...
test('literal child messages do not execute slash commands or expand even the assigned skill',{timeout:15000},async t=>{
  ...
  let ready: () => void = () => undefined,release: () => void = () => undefined,calls=0;const started=new Promise<void>(resolve=>{ready=resolve;});
  const literal='/skill:dev-explore this is a literal message\nPreserve this second line.';
  runtime.streamSimple=(model,context,options)=>{
    const stream=createAssistantMessageEventStream();
    const response=(text: string): AssistantMessage=>({role:'assistant',content:[{type:'text',text}],stopReason:'stop',timestamp:Date.now(),provider:model.provider,model:model.id,api:model.api,usage:{input:1,output:1,totalTokens:2,cacheRead:0,cacheWrite:0,cost:{input:0,output:0,cacheRead:0,cacheWrite:0,total:0}}});
    const finish=(text: string): void=>stream.push({type:'done',reason:'stop',message:response(text)});
  ...
  const worker=hub.list()[0];assert.ok(worker);const receipt=await hub.send(worker.id,literal);release();
