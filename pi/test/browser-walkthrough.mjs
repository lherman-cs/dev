...
try {
  ...
  assert.match(await evaluate("document.querySelector('main h1')?.textContent"), /Await the first published assessment/);
  ...
  await click("Inspect changed subjects");
  await until(() => evaluate("[...document.querySelectorAll('button')].find(b=>b.textContent.includes('Apply inspected'))?.disabled === false"), "revision inspected");
  ...
  assert.match(await evaluate("document.querySelector('[aria-label^=\"evidence for\"]')?.innerText"), /retention policy is not tested/);
  await key("Escape");
  await until(() => evaluate("!document.querySelector('[aria-label^=\"evidence for\"]')"), "detail closed");
  assert.match(await evaluate("document.querySelector('main h1')?.textContent"), /Retention policy gap/);
  await click("Accept recommendation");
  await click("Confirm and continue");
  await until(() => evaluate("document.querySelector('main h1')?.textContent === 'Submit your review'"), "automatic progression to completion");
  assert.match(await evaluate("document.querySelector('[aria-label=\"Remaining work\"]')?.innerText"), /0 decisions left/);
  await click("Approve review");
  ...
  console.log("Browser walkthrough passed: revision inspection, one-item progression, on-demand detail, approval, mobile width.");
} finally {
