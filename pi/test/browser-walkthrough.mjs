...
try {
  ...
  assert.match(await evaluate("document.querySelector('main h1')?.textContent"), /Who owns the next action/);
  assert.equal(await evaluate("document.querySelectorAll('main h1').length"), 1);
  assert.ok(await evaluate("Boolean(document.querySelector('#own-answer'))"), "first question has an inline custom answer");
  assert.match(await evaluate("document.querySelector('main')?.innerText"), /Require acceptance/);
  await evaluate("document.querySelector('#own-answer').focus()");
  await send("Input.insertText", { text: "Keep the previous owner until acceptance." });
  await pause(2700);
  assert.equal(await evaluate("document.querySelector('#own-answer')?.value"), "Keep the previous owner until acceptance.", "polling preserves the typed answer");
  await click("Send to agent");
  await until(() => evaluate("document.querySelector('main')?.innerText.includes('answer is with the agent')"), "answer acknowledged in place");
  assert.equal(await evaluate("document.querySelector('main h1')?.textContent"), "Who owns the next action?");
  assert.equal(await evaluate("document.querySelector('main')?.innerText.includes('Accept recommendation')"), true, "sending does not accept consent");
  await visit(urls.Spec);
  assert.match(await evaluate("document.querySelector('main h1')?.textContent"), /Handoff history/);
  await click("Inspect what changed");
  ...
  await click("See how this works");
  ...
  assert.match(await evaluate("document.querySelector('main')?.innerText"), /0 decisions to answer/);
