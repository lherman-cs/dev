...
function required<T>(value: T | undefined, label: string): T { assert.ok(value,`Missing ${label}`); return value; }
...
test('daily flow: every surface preserves exit/help on narrow and short viewports',t=>{
  ...
  for(const [width,height] of [[120,40],[80,24],[40,12],[22,8],[12,4]] as const) {
...
test('daily flow: a too-narrow thread cannot accept invisible typing',t=>{
  ...
  f.view.handleInput(keys.enter);screen(f.view,12);f.view.handleInput('invisible');assert.equal(required(hub.get('a'),'worker a').draft,'');
});
...
test('daily flow: a long multiline paste keeps all 30 Unicode lines',async t=>{
  ...
  const call=required(s.calls[0],'steer call');assert.equal(call[0],'steer');assert.equal(call[1],text);
});
...
test('daily flow: coalesced updates and repeated views retain one observer and bounded repaint',async t=>{
  ...
  for(let i=0;i<3000;i++) { const message=assistant(`stream ${i}`);s.emit({type:'message_update',message,assistantMessageEvent:{type:'text_delta',contentIndex:0,delta:'',partial:message}}); }
  ...
  assert.equal(s.listenerCount(),1);const partial=required(required(hub.get('a'),'worker a').partial,'partial');assert.ok(partial.role==='assistant');const text=required(partial.content[0],'partial text');assert.ok(text.type==='text');assert.equal(text.text,'stream 2999');
});

test('daily flow: native edit diff is inspectable without executing a tool',t=>{
  const hub=new WorkerHub(),s=session(),record=register(hub,s,'a');const f=viewFixture(t,{hub});
  const call: AssistantMessage={...assistant(''),content:[{type:'toolCall',id:'edit',name:'edit',arguments:{path:'not-a-real-file.rs',edits:[{oldText:'old',newText:'NEW_DIFF_CANARY'}]}}]};
  const toolResult: ToolResultMessage={role:'toolResult',toolCallId:'edit',toolName:'edit',content:[{type:'text',text:'Updated'}],details:{diff:'-1 old\n+1 NEW_DIFF_CANARY',patch:'-old\n+NEW_DIFF_CANARY',firstChangedLine:1},isError:false,timestamp:Date.now()};
  s.append(call);s.append(toolResult);
  const renderer=new NativeTranscript(f.tui,record);renderer.setExpanded(true);t.after(()=>{renderer.dispose();hub.dispose();});
