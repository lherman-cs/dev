...
* When `wait_agent` reports completion, immediately inspect and reconcile that child's final output before waiting, spawning, or advancing.
* Never call `wait_agent` again for a completed child or describe it as still awaiting/pending.
* If a completed child has no recognized terminal result, treat it as an invalid result, not as still running.
* On resume/compaction, reconcile known completed children before spawning replacements.
...
* Reviewer completion without a recognized verdict -> retry the same immutable review once with a fresh Reviewer; do not modify the candidate or consume repair budget.
* If that replacement Reviewer also completes without a recognized verdict -> human escalation; do not spawn a third Reviewer.
* A second unresolved valid review -> Planner for invalid assumptions, otherwise human escalation; NO automatic repair #2.
