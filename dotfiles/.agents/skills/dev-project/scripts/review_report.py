...
MODES = {'task', 'repair', 'final', 'final-repair'}
...
REPORT_KEYS = {'schema', 'mode', 'base', 'candidate', 'contract_sha256', 'verdict',
               'findings', 'resolutions', 'checked', 'blocker'}
PACKET_KEYS = {'schema', 'candidate', 'contract_sha256', 'findings', 'seen_ids'}
...
def _packet(path: Path, *, base: str, contract: str) -> dict:
    ...
    require(type(packet['schema']) is int and packet['schema'] == 1, 'Unsupported packet schema')
...
def validate(path: Path, *, base: str, candidate: str, contract: Path, mode: str,
             previous: Path | None = None) -> tuple[dict, dict | None]:
    ...
    require(type(report['schema']) is int and report['schema'] == 1, 'Unsupported review schema')
    ...
    if is_repair:
        ...
        packet = _packet(previous, base=base, contract=contract_hash)
        ...
        for item in report['resolutions']:
            ...
            require(item['status'] in {'RESOLVED', 'UNRESOLVED'}, 'Invalid resolution status')
        ...
        require(all(f['origin'] in {'repair', 'late-discovery'} for f in new),
    ...
    packet = None
    if active:
        packet = {'schema': 1, 'candidate': candidate, 'contract_sha256': contract_hash,
                  'findings': active, 'seen_ids': sorted(seen)}
    envelope = {'status': 'VALID', 'verdict': report['verdict'], 'candidate': candidate,
                'report': str(path), 'blocking_ids': [f['id'] for f in active],
